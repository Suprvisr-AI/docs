...
description: Connect Google Drive so Suprvisr AI can search and reference your documents with per-user permission enforcement.
---
...
Connect Suprvisr to your Google Drive shared drives or personal drives. Once connected, the AI can search and reference your organisation's documents — and results are filtered by each user's actual Google Drive permissions.
...
With Google Drive connected, Suprvisr can:
...
Click **Connect Google Drive**. You'll be redirected to Google to grant Suprvisr read-only access to your drives.
...
Review your selection and click **Connect**. Suprvisr begins indexing your documents immediately.
...
1. Suprvisr finds relevant document chunks using semantic search
2. For each unique file, Suprvisr calls the Google Drive API with **your** access token
